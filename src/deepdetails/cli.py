...
def _run_export_pred(args: argparse.Namespace):
    ...
    export_results(
        model=DeepDETAILS, **_args_to_kwargs(args, {"hide_progress_bar", "version"})
    )
...
def _run_merge_preds(args: argparse.Namespace):
    ...
    merge_rep_preds(
        in_pred_files=pred_files,
        save_to=args.save_to,
        keep_old=args.keep_old_preds,
        quiet=args.quiet,
    )
...
def _general_parser(parent_parser: argparse.ArgumentParser):
    ...
    group.add_argument(
        "--study-name", default="DeepDETAILS", type=str, help=PARAM_DESC["study_name"]
    )
    group.add_argument(
        "--save-to", default=".", type=created_dir, help=PARAM_DESC["save_to"]
    )
    group.add_argument(
        "--num-workers", help=PARAM_DESC["num_workers"], type=int, default=16
    )
    group.add_argument(
        "--batch-size", help=PARAM_DESC["batch_size"], type=int, default=32
    )
    group.add_argument(
        "--hide-progress-bar", action="store_true", help=PARAM_DESC["hide_progress_bar"]
    )
...
def _training_parser(parent_parser: argparse.ArgumentParser):
    ...
    group.add_argument(
        "--dataset", required=True, type=existing_dir, help=PARAM_DESC["dataset"]
    )
    ...
    group.add_argument(
        ...
        dest="cv",
        default=("chr22",),
        help=PARAM_DESC["chromosomal_validation"],
        nargs="*",
    )
    group.add_argument(
        "--chromosomal-testing",
        ...
        dest="ct",
        default=("chr19",),
        help=PARAM_DESC["chromosomal_testing"],
        nargs="*",
    )
    group.add_argument(
        "--accelerator",
        type=str,
        default="auto",
        choices=("gpu", "tpu", "auto", "cpu", "ipu"),
        help=PARAM_DESC["accelerator"],
    )
    group.add_argument(
        "--devices",
        help=PARAM_DESC["devices"],
        nargs="+",
    ...
    group.add_argument(
        "--earlystop-patience",
        help=PARAM_DESC["earlystop_patience"],
        type=int,
        default=2,
    )
    group.add_argument(
        "--min-delta", help=PARAM_DESC["min_delta"], type=float, default=0.0001
    )
    group.add_argument(
        "--max-epochs", help=PARAM_DESC["max_epochs"], type=int, default=50
    )
    group.add_argument(
        "--save-top-k-model", help=PARAM_DESC["save_top_k_model"], default=1, type=int
    )
    group.add_argument(
        "--resume",
        "--resume-from-ckpt",
        dest="resume_from_ckpt",
        help=PARAM_DESC["resume_from_ckpt"],
        type=str,
    )
    # for backward compatibility
    g = group.add_mutually_exclusive_group()
    g.add_argument(
        "--save-preds", action="store_true", default=True, help=PARAM_DESC["save_preds"]
    )
    g.add_argument(
        "--no-preds",
        action="store_false",
        dest="save_preds",
        help=PARAM_DESC["no_preds"],
    )
    group.add_argument(
        "--alpha",
        "--redundancy-loss-coef",
        help=f"{PARAM_DESC['redundancy_loss_coef']} (0-100 scale; divided by 100 internally)",
        type=alpha_as_fraction,
        default="1",
        dest="redundancy_loss_coef",
        metavar="{0..100}",
    )
    group.add_argument(
        "--prior-loss-coef", help=PARAM_DESC["prior_loss_coef"], type=float, default=1.0
    )
    group.add_argument("--gamma", help=PARAM_DESC["gamma"], type=float, default=1e-8)
    group.add_argument(
        "--learning-rate", help=PARAM_DESC["learning_rate"], type=float, default=1e-3
    )
    group.add_argument(
        "--lr-step-size", help=PARAM_DESC["lr_step_size"], type=int, default=1
    )
    group.add_argument(
        "--lr-gamma", help=PARAM_DESC["lr_gamma"], type=float, default=0.1
    )
    group.add_argument(
    ...
    group.add_argument(
        "--rescaling-mode",
        dest="rescaling_mode",
        help=PARAM_DESC["rescaling_mode"],
        choices=RescalingMode.values(),
        default=RescalingMode.COUNTS,
        type=RescalingMode.parse,
    )
    # for backward compatibility
    g = group.add_mutually_exclusive_group()
    g.add_argument(
    ...
    group.add_argument(
        ...
        "--version-tag",
        dest="version",
        help=PARAM_DESC["wandb_version"],
        type=str,
        default="",
    )
    group.add_argument(
        "--loads-trunc", required=False, type=int, help=PARAM_DESC["loads_trunc"]
    )
...
def _model_conf_parser(parent_parser: argparse.ArgumentParser):
    ...
    group.add_argument(
        "--profile-shrinkage",
        help=PARAM_DESC["profile_shrinkage"],
        type=int,
        default=8,
        required=False,
    )
    ...
    group.add_argument(
        ...
        type=int,
        default=3,
        help=PARAM_DESC["head_mlp_layers"],
    )
    group.add_argument(
        "--gru-layers", help=PARAM_DESC["gru_layers"], type=int, default=2
    )
    group.add_argument(
        "--gru-dropout", help=PARAM_DESC["gru_dropout"], type=float, default=0.1
    )
    group.add_argument(
        "--scale-function-placement",
        choices=("early", "late", "late-ch", "disable"),
        help=PARAM_DESC["scale_function_placement"],
        default="late-ch",
    )
    group.add_argument(
        "--seq",
    ...
    group.add_argument(
        "--n-times-more-embeddings",
        help=PARAM_DESC["n_times_more_embeddings"],
        type=int,
        default=2,
    )
...
def _prep_dataset_parser(parent_parser: argparse.ArgumentParser):
    ...
    group.add_argument(
        "--bulk-pl", required=True, type=existing_file, help=PARAM_DESC["bulk_pl"]
    )
    ...
    group.add_argument(
        "--genome-fa", type=existing_file, required=True, help=PARAM_DESC["genome_fa"]
    )
    group.add_argument(
        "--chrom-size", type=existing_file, required=True, help=PARAM_DESC["chrom_size"]
    )
    group.add_argument("--seed", type=int, default=1234567, help=PARAM_DESC["seed"])
    group.add_argument(
        "--ref-labels", nargs="*", type=str, help=PARAM_DESC["ref_labels"]
    )
    group.add_argument(
        "--ref-pls", nargs="*", type=existing_file, help=PARAM_DESC["ref_pls"]
    )
    group.add_argument(
        "--ref-mns", nargs="*", type=existing_file, help=PARAM_DESC["ref_mns"]
    )
    group.add_argument(
        "--background-sampling-ratio",
        help=PARAM_DESC["background_sampling_ratio"],
        default=0.0,
        type=float,
    )
    group.add_argument(
    ...
    group.add_argument(
        "--final-regions", action="store_true", help=PARAM_DESC["final_regions"]
    )
    group.add_argument(
        "--keep-frags", action="store_true", help=PARAM_DESC["keep_frags"]
    )
    group.add_argument(
        "--disable-rpm", action="store_true", help=PARAM_DESC["disable_rpm"]
    )
    group.add_argument(
        "--memory-saving", action="store_true", help=PARAM_DESC["memory_saving"]
    )
    group.add_argument(
        "--collapse-missing-cell-types",
        action="store_true",
        help=PARAM_DESC["collapse_missing_cell_types"],
    )
    group.add_argument(
        "--merge-overlap-peaks",
        help=PARAM_DESC["merge_overlap_peaks"],
        type=int,
        default=0,
    )
    group.add_argument(
        "--target-sliding-sum",
        help=PARAM_DESC["target_sliding_sum"],
        default=0,
        type=int,
    )
...
def _preflight_parser(parent_parser: argparse.ArgumentParser):
    ...
    group0.add_argument(
        "--skip-preflight", action="store_true", help=PARAM_DESC["skip_preflight"]
    )
    group0.add_argument(
        "--combine-cell-types",
        type=str,
        nargs="*",
        help=PARAM_DESC["combine_cell_types"],
    )
    group.add_argument(
        "--accessible-regions",
        dest="accessible_peaks",
        required=False,
        type=existing_file,
        help=PARAM_DESC["accessible_peaks"],
    )
    group.add_argument(
        "--preflight-cutoff",
        help=PARAM_DESC["preflight_cutoff"],
        default=0.035,
        type=float,
    )
    group.add_argument(
        "--nu", help=PARAM_DESC["preflight_nu"], default=0.85, type=float
    )
    group.add_argument("--use-qnorm", action="store_true", help=PARAM_DESC["use_qnorm"])
    group.add_argument(
        "--candidate-qval", help=PARAM_DESC["qval_cutoff"], default=0.01, type=float
    )
    group.add_argument(
        "--candidate-fc", help=PARAM_DESC["fc_cutoff"], default=2.0, type=float
    )
    group.add_argument(
        "--max-top-n", help=PARAM_DESC["max_top_n"], type=int, default=1000
    )
    group.add_argument(
        "--min-cells-required",
        help=PARAM_DESC["min_cells_required"],
        type=int,
        default=20,
    )
    group.add_argument("--n-aggs", help=PARAM_DESC["n_aggs"], type=int, default=5)
...
def _export_pred_parser(parent_parser: argparse.ArgumentParser):
    ...
    group.add_argument(
        "--merge-strands", action="store_true", help=PARAM_DESC["merge_strands"]
    )
...
def _export_wg_pred_parser(parent_parser: argparse.ArgumentParser):
    ...
    group.add_argument(
        "--bulk-mn",
        dest="mn_bulk_bw_file",
        type=existing_file,
        help=PARAM_DESC["bulk_mn"],
    )
    group.add_argument(
        "--accessibility",
        dest="acc_bw_files",
...
def _build_bw_parser(parent_parser: argparse.ArgumentParser):
    ...
    group.add_argument(
        "-s", "--save-to", default=".", type=created_dir, help=PARAM_DESC["save_to"]
    )
    group.add_argument(
        "-c",
        "--chrom-size",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser_deconv = subparsers.add_parser(
        "deconv",
        help="Using DeepDETAILS to deconvolve a bulk sample",
    ...
    parser_export_pred = subparsers.add_parser(
        "export-pred",
        help="Export predictions to a hdf5 file",
        formatter_class=_Formatter,
    )
    ...
    parser_gw_export_pred = subparsers.add_parser(
        "export-gw-pred",
        help="Export whole-genome predictions to a hdf5 file",
        formatter_class=_Formatter,
    )
    ...
    parser_merge_preds = subparsers.add_parser(
        "merge-preds",
        help="Merge predictions from multiple replicate runs",
        formatter_class=_Formatter,
    )
